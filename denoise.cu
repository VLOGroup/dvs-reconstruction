...
/// Dual Kernel for manifold denoising with occurence dependent lambda (shared between all data terms)
__global__ void TV_manifold_occurence_lambda_dual_kernel(iu::ImageGpu_32f_C4::KernelData p, iu::ImageGpu_32f_C4::KernelData m, cudaTextureObject_t tex_u,
                                                     iu::ImageGpu_32u_C1::KernelData o, float lambda, float sigma)
{
    unsigned int x = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int y = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    if (x < p.width_ && y < p.height_)
    {
        ...
        // calc data dependent lambda
        lambda*=o(x,y)+1;

        pc += sigma*p_upd;
        pc /= 1+ sigma*0.2f; // Huberize
        pc /= fmaxf(1.f,sqrtf(sqr(pc.x)+sqr(pc.y)+sqr(pc.z))/det*lambda);
        p(x,y) = pc;
    }
}

/// Dual Kernel for manifold denoising with time dependent lambda (shared between all data terms)
__global__ void TV_manifold_delta_lambda_dual_kernel(iu::ImageGpu_32f_C4::KernelData p, iu::ImageGpu_32f_C4::KernelData m, cudaTextureObject_t tex_u,
                                                     float curr_time, float lambda, float sigma)
{
    unsigned int x = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int y = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    if (x < p.width_ && y < p.height_)
    {
        ...
        // calc data dependant lambda
        lambda*=min(curr_time-tc,1.f);
...
//    L = sqrtf(12.0f);
    ...
    for (int k = 1; k < iterations; k++)
    {
        TV_manifold_delta_lambda_dual_kernel<<<dimGrid, dimBlock>>>(*p_manifold, *manifold_t, u_->getTexture(), curr_time, lambda, sigma);
        switch(method)
        {
            case TV_L1:
                tau = 1/L;
...
//    L = sqrtf(12.0f);
    ...
    for (int k = 1; k < iterations; k++)
    {
        TV_manifold_occurence_lambda_dual_kernel<<<dimGrid, dimBlock>>>(*p_manifold, *manifold_t, u_->getTexture(), *o, lambda, sigma);
        switch(method)
        {
            case TV_L1:
                tau = 1/L;
